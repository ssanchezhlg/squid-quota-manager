...
logging.basicConfig(filename=os.path.join(LOGDIR, 'pquot-reset.log'), level=logging.INFO, format='[ %(asctime)s ] %(message)s')
...
try:     
   ...
   # db = pymysql.connect(host=dbhost, user=dbuser, password=dbpasswd, db=dbname)
    cursor = db.cursor()

    UPDATE_QUERY = "UPDATE quota SET used = 0"
